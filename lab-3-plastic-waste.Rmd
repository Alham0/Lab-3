...
```
Answer:the africa is the most density continent and the south america is the most continent in plastic wate per capl
...
ggplot(data = plastic_waste,
    ...
    geom_point()
```
answer: Europe ( Positive ) relationship , Africa is the most (mismanged - plastic waste per cap )
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
Answer: gemo_smooth (Aids the eye in seeing patterns in the presence of overplotting also it's help us to do see the curve of the data and analyis it via a different way ) the coastal cites in asia and north america is have the most plastic waste per capita and the lowest population
